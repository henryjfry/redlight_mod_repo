...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.redlight_mod').getAddonInfo(info)
...
def addon_fanart():
	return (
		...
		or 'special://home/addons/plugin.video.redlight_mod/resources/media/fanart.jpg'
	)
...
def resolve_list_icon(icon, default_name='folder'):
	...
	if icon_norm.startswith('special://') or 'plugin.video.redlight_mod/resources/media/' in icon_norm:
...
def build_url(url_params):
	return 'plugin://plugin.video.redlight_mod/?%s' % urlencode(url_params)
...
def sanitize_folder_url(url):
	if not url or 'plugin.video.redlight_mod' not in url: return url
	try:
...
def addon(addon_id='plugin.video.redlight_mod'):
...
def run_addon(addon='plugin.video.redlight_mod', block=False):
...
def schedule_widget_refresh(silent=True, reload_skin=False):
	url = 'plugin://plugin.video.redlight_mod/?mode=refresh_widgets&silent=%s&reload_skin=%s' % ('true' if silent else 'false', 'true' if reload_skin else 'false')
...
def disable_enable_addon(addon_name='plugin.video.redlight_mod'):
	import json
	try:
...
def sync_addon_xml_from_settings(addon_name='plugin.video.redlight_mod'):
...
def update_kodi_addons_db(addon_name='plugin.video.redlight_mod'):
	import time
